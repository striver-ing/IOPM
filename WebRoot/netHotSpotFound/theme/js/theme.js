$(function(){
    ...
    '<div class="sidebar-R"></div></div></li>';
                  ...
                  //滚动条
                    ...
                    var $scrollBox = parseInt($(".submenu").eq(index).css('width'));
                    var content = $('.submenu-z').eq(index);
                    ...
                    if($content > 1022){
                        ...
                        bar.mousedown(function (event) {
                            ...
                            document.onmousemove = function (event) {
                                ...
                                if(x>$scrollBox-barWidth){
                                    x = $scrollBox-barWidth;
                                }
                                //赋值给小bar就可以了
                                bar.offset({left:x});
                                //需求3：小bar移动，content部分等比例移动
                                var bili2 = ($content - $scrollBox)/($scrollBox - barWidth);
                                var cony = bili2 * x;
                                //为content走过的距离赋值。
                                content.css('margin-left',-cony);
